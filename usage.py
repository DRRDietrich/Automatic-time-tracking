...
def summarize():
    with open(log, "wt" ) as report:
        ...
        report.write("")
        for x in range(0, numberOfWorkspaces):
            report.write(workspaceNameList[x] + ": " + time_format(wslist[x]) + "\n")
        report.write("\n")
        alldata = []      
        for app in applist:
            appdata = []; windata = []
...
t = 0; applist = []; winlist = [];
numberOfWorkspaces = int(subprocess.check_output("gsettings get org.gnome.desktop.wm.preferences num-workspaces", shell=True).decode('utf-8').strip())
workspaceNameList = subprocess.check_output("gsettings get org.gnome.desktop.wm.preferences workspace-names | sed \"s/', '/,/g;s/\['//g;s/'\]//g\"", shell=True).decode('utf-8').strip().split(',')
wslist = [0] * numberOfWorkspaces
...
while True:
    ...
    if not frname in checklist:
        ...
        winlist[checklist.index(frname)][2] = winlist[checklist.index(frname)][2]+1*period
    wslist[workspace] = wslist[workspace] + 1*period
    if t == 60/period:
        summarize()
        t = 0
    else:
